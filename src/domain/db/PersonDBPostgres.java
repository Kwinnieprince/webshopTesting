...
public class PersonDBPostgres implements PersonDb {
    ...
    public void add(Person person) throws UnsupportedEncodingException, NoSuchAlgorithmException {
        ...
        //String sql = "INSERT INTO r0712411web.persons VALUES('" + person.getUserid() + "','" + person.getEmail() + "','" + person.getPassword() +"','" + person.getFirstName() +"','" + person.getLastName() + "')";
        String sql = "INSERT INTO r0712411web.persons VALUES(?,?,?,?,?)";
        try(Connection connection = DriverManager.getConnection(url, properties)){
    ...
    public void update(Person person) throws UnsupportedEncodingException, NoSuchAlgorithmException {
        ...
        String sql = "UPDATE r0712411.products SET userid=?, email=?, pass=?, firstname=?, lastname=?";
        try(Connection connection = DriverManager.getConnection(url, properties);
            PreparedStatement statement = connection.prepareStatement(sql)) {
